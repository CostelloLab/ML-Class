...
expression.data <- read.delim("../Projects/Code/DS-transfer/data/HTP_WholeBlood_RNAseq_FPKMs_Synapse.txt")
expression.data <- expression.data %>%
    select(LabID, Gene_name, Value) %>%
    group_by(LabID) %>%
    mutate(row = row_number()) %>%    
    pivot_wider( names_from =c("Gene_name","row"), values_from = "Value")
expression.data <- expression.data %>%
...
## tried tidyverse, but was taking much longer
...
### added tghe tidyverse format, but it's super slow. why??
...
### TODO: Impute missing data with 2 other algorithms ###
  ...
  imputed.min <- apply(expression.temp[,which(apply(expression.temp, 2, function(x) any(is.na(x))))], 2, min)
  imputed.median <- apply(expression.temp[,which(apply(expression.temp, 2, function(x) any(is.na(x))))], 2, median)
...
errors %>% 
  ggplot(aes(x = thresholds, y = mse)) +
  geom_line(color = "skyblue") +
  geom_point(color = "skyblue", fill = "white", size = 3, shape = 21) +
