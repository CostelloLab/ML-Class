...
rankGenes <- function(data, statistic = "variance") {
    if(statistic == "variance") {
        ...
        ranks <- data %>%
            summarise(across(everything(), CV)) %>%
            sort(decreasing = TRUE) %>%
            names()
    }
    return(ranks)
...
var.ranks <- rankGenes(expression.data, statistic = "variance")
cv.ranks <- rankGenes(expression.data, statistic = "cv") ## not the same length as var.ranks. Dividing by 0


### TODO: Summarize and compare top gene lists from by var and c.v. methods above ###
var.rank <- as.data.frame(cbind(gene = var.ranks,var_rank = 1:length(var.ranks)))
cv.rank <- as.data.frame(cbind(gene = cv.ranks, cv_rank = 1:length(cv.ranks))) 
ranks <- var.rank %>%
...
MSE <- function(yhat, y){
    squared.error <- sum(mapply(function(yhat,y) (yhat-y)^2, yhat, y), na.rm = T)
...
# restrict to the 5,000 genes with top CV. 
expression.data.var <- expression.data[var.ranks[1:5000]]

for (percent in seq(.01, .50, by = .05)){

    expression.temp <- t(expression.data.var) # transpose for knn functions

    ## Delete data based on percentag
    n_cols <-  ncol(expression.temp)
    n_rows <- nrow(expression.temp)

    total_pairs <- n_cols * n_rows
    n_pairs_to_delete <- floor(percent * total_pairs)

    rows_to_delete <- sample(1:n_rows, n_pairs_to_delete, replace = T)
    cols_to_delete <- sample(1:n_cols, n_pairs_to_delete, replace = T)

    values_to_delete <- numeric()
    pairs_to_delete <- character()
    
    for(i in 1:n_pairs_to_delete) {
        pair <- paste(rows_to_delete[i], cols_to_delete[i])
        if(!(pair %in% pairs_to_delete)) {
            values_to_delete[i] <- expression.temp[rows_to_delete[i], cols_to_delete[i]]
            expression.temp[rows_to_delete[i], cols_to_delete[i]] <- NA
        }
        pairs_to_delete <- c(pair, pairs_to_delete)
    }
    
      
  # Impute missing data with knn algorithm
  imputed.data <- impute.knn(expression.temp, k =10, maxp = n_rows, rng.seed = 1234)
  imputed.values <- numeric()

    for(i in 1:n_pairs_to_delete) {
        imputed.values[i] <- imputed.data$data[rows_to_delete[i], cols_to_delete[i]]
    }
...
### TODO: Impute missing data with 2 other algorithms ###
    ...
    imputed.min <- mapply(function(row, col) 
                           return(min(expression.temp[row,], na.rm = T)), 
                        row = rows_to_delete , col = cols_to_delete)
    imputed.median <- mapply(function(row, col) 
                           return(median(expression.temp[row,], na.rm = T)), 
                        row = rows_to_delete , col = cols_to_delete)
    
  
  
  # Evaluate performance with MSE
  mse <- MSE(imputed.values, values_to_delete)
    min.mse <- MSE(imputed.min, values_to_delete)
    median.mse <- MSE(imputed.median,values_to_delete )
